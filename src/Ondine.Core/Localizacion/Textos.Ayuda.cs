...
public sealed partial class Textos
{
    ...
    public string AyudaOrgPasosTipo => Idioma.Elegir(
        "«Library»: say whether the folder holds a TV series or films, and the rest of the tab follows. A series is identified against a catalogue, with seasons and episode numbers. A film has none of that — there is no episode list to build a catalogue from, because a film is just a film — so with «Films» selected, «Analyse» opens its own window: it shows what would change so each film ends up as «Title (Year)/Title (Year).ext», which is what Plex and Jellyfin expect, and nothing is touched until you accept. A folder holding several films is treated as a collection and is NOT taken apart: only the file names inside are cleaned up. And if you turn on TMDb in Preferences, Films, the window gets a button to identify them against that database: it gets the title and the year right, it knows that «The commuter» and «El pasajero» are the same film, and whatever it cannot identify with confidence is shown and left alone — a film identified wrongly is worse than one not identified at all. Only the cleaned-up title and the year leave your computer, never the file name. The choice is remembered per folder.",
        "«Biblioteca»: di si la carpeta tiene una serie o películas, y el resto de la pestaña se adapta. Una serie se identifica contra un catálogo, con temporadas y números de episodio. Una película no tiene nada de eso —no hay anexo de episodios del que sacar un catálogo, porque una película es solo una película—, así que con «Películas» puesto, «Analizar» abre su propia ventana: enseña qué cambiaría para que cada una acabe como «Título (Año)/Título (Año).ext», que es lo que esperan Plex y Jellyfin, y no toca nada hasta que aceptas. Una carpeta con varias películas dentro se trata como colección y NO se desmonta: solo se limpian los nombres de los ficheros. Y si enciendes TMDb en Preferencias, Películas, la ventana trae un botón para identificarlas contra esa base de datos: acierta el título y el año, sabe que «The commuter» y «El pasajero» son la misma película, y lo que no pueda identificar con confianza lo enseña y no lo toca —una película mal identificada es peor que una sin identificar—. De tu equipo solo sale el título ya limpio y el año, nunca el nombre del fichero. Lo elegido se recuerda por carpeta.");
