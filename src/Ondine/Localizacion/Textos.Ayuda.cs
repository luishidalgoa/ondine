...
public sealed partial class Textos
{
    ...
    public string AyudaOrgPasosTipo => Idioma.Elegir(
        "«Library»: say whether the folder holds a TV series or films, and the rest of the tab follows. A series is identified against a catalogue, with seasons and episode numbers. A film has none of that — there is no episode list to build a catalogue from, because a film is just a film — so its name is built from the title and the year read from the filename itself, and it goes to «Title (Year)/Title (Year).ext», which is what Plex and Jellyfin expect. The choice is remembered per folder. Identifying films against a public database is not in yet.",
        "«Biblioteca»: di si la carpeta tiene una serie o películas, y el resto de la pestaña se adapta. Una serie se identifica contra un catálogo, con temporadas y números de episodio. Una película no tiene nada de eso —no hay anexo de episodios del que sacar un catálogo, porque una película es solo una película—, así que su nombre se compone con el título y el año leídos del propio fichero, y va a «Título (Año)/Título (Año).ext», que es lo que esperan Plex y Jellyfin. Lo elegido se recuerda por carpeta. Identificarlas contra una base de datos pública todavía no está.");
