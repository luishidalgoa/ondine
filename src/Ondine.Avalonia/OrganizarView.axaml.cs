...
public partial class OrganizarView : UserControl
{
    ...
    /// <summary>
    /// Un pincel del tema por su nombre. En WPF era <c>FindResource</c>, que lanza si no
    /// está; aquí se pregunta y se cae a gris — lo que se pierde es el color de una
    /// insignia, no la pantalla.
    /// </summary>
    ...
    /// <summary>
    /// La vista de la tabla: lo que filtra y ordena sin tocar la colección de abajo.
    ///
    /// <para>
    /// En WPF esto era <c>CollectionViewSource.GetDefaultView(_filas)</c>, que en Avalonia
    /// <b>no existe</b>. El equivalente es <c>DataGridCollectionView</c>, con una diferencia
    /// que importa: allí la vista se sacaba de la colección y el DataGrid la encontraba solo;
    /// aquí <b>hay que dársela como ItemsSource</b>, y si se le da la colección a pelo el
    /// filtro no se aplica y no hay ningún error.
    /// </para>
    /// </summary>
    ...
    public OrganizarView()
    {
        ...
        btnConfCancelar.Click += (_, _) => overlayConfirmar.IsVisible = false;
        btnConfAceptar.Click += (_, _) => { overlayConfirmar.IsVisible = false; Aplicar(); };
        ...
        Loaded += (_, _) =>
        {
            ...
            if (Ventana is { } w)
                w.Activated += (_, _) =>
                {
                    if (vistaInicio.IsVisible && !_cargando)
                        CargarCatalogos();
                };
        };
    }
    ...
    private void CargarCatalogos()
    {
        ...
        if (_catalogoElegido != null && string.IsNullOrWhiteSpace(txtCarpeta.Text))
        {
            ...
            if (cs.Count > 0)
            {
                ...
                Dispatcher.UIThread.Post(new Action(RevisarCarpeta),
                    DispatcherPriority.Background);
            }
        }

        cboSerie.SelectedItem = _catalogoElegido;
        panelSinCatalogos.IsVisible = _catalogos.Count == 0;
        PintarTarjetas();
    ...
    {
        if (sender is Control fe && fe.Tag is string ruta)
    ...
    /// </summary>
    private async void OnQuitarCatalogo(object sender, RoutedEventArgs e)
    {
        if (sender is not Control fe || fe.Tag is not string ruta) return;
        ...
        if (!await Dialogo.Confirmar(Ventana, Textos.Instancia.OrganizarQuitarCatalogoTitulo,
    ...
    private void AplicarTipo()
    {
        ...
        panelSerie.IsVisible = Ver(esSerie);
        panelPlantilla.IsVisible = Ver(esSerie);
        filaAcciones.IsVisible = Ver(esSerie);

        vistaInicio.IsVisible = Ver(esSerie && !revisando);
        vistaRevision.IsVisible = Ver(esSerie && revisando);
        filaChips.IsVisible = Ver(esSerie && revisando);

        vistaPeliculas.IsVisible = Ver(!esSerie && !revisando);
        vistaRevisionPelis.IsVisible = Ver(!esSerie && revisando);
        filaChipsPelis.IsVisible = Ver(!esSerie && revisando);
        filaAccionesPelis.IsVisible = Ver(!esSerie && revisando);

        if (!esSerie && !revisando) vistaPeliculas.Poner(CarpetaActual(), _ficheros);
    }

    /// <summary>
    /// En WPF esto convertía a Visibility; aquí IsVisible ya es booleano, así que el
    /// ayudante se queda por no tocar los cincuenta sitios que lo llaman — y porque leerse
    /// «Ver(hay)» sigue diciendo más que un booleano suelto.
    /// </summary>
    private static bool Ver(bool si) => si;
    ...
    private async void Simular()
    {
        ...
        bool animar = vistaInicio.IsVisible;
        ...
        if (animar)
        {
            panelReposo.IsVisible = false;
            panelEtapas.IsVisible = true;
            _pasos.Reiniciar();
            EncenderHaz();
        }
        ...
        finally
        {
            ...
            {
                ApagarHaz();
                panelEtapas.IsVisible = false;
                panelReposo.IsVisible = true;
            }
    ...
    /// <summary>
    /// La espera mientras identifica.
    ///
    /// <para>
    /// En WPF esto encendía además un «haz» giratorio por el borde del panel: un degradado
    /// animado con <c>BeginAnimation</c> sobre el ángulo de una transformación. Eso no se
    /// porta —es un adorno que aquí costaría una animación escrita a mano— y de paso se va
    /// con él la trampa que traía: la animación era <c>RepeatBehavior.Forever</c>, y una que
    /// se quedara viva mantiene su objeto en pie y obliga a pintar en cada fotograma. Es
    /// exactamente el fallo que costó un 11 % de núcleo en el reproductor.
    /// </para>
    /// <para>
    /// Lo que hacía falta —decir que está trabajando— lo dicen los círculos de espera y el
    /// rótulo, que sí están.
    /// </para>
    /// </summary>
    ...
    private void MostrarInicio()
    {
        _filas.Clear();
        _fase = Fase.Inicio;
        bannerAplicado.IsVisible = false;
        AplicarTipo();
    }
    ...
    private void ActualizarContadores()
    {
        ...
        if (r.AvisarDeDudas)
        {
            ...
            bannerAviso.IsVisible = true;
        }
        else bannerAviso.IsVisible = false;
    }
    ...
    private void OnTablaClic(object sender, PointerPressedEventArgs e)
    {
        ...
        if (Ascender<CheckBox>(e.Source as Visual) is
        ...
        var celda = Ascender<DataGridCell>(e.Source as Visual);
    ...
    /// <summary>
    /// Sube por el árbol visual hasta encontrar un ancestro de ese tipo.
    ///
    /// <para>
    /// En WPF esto recorría <c>DependencyObject</c> con <c>VisualTreeHelper.GetParent</c>;
    /// aquí es <c>Visual</c> y <c>GetVisualParent()</c>. Es la misma idea con otros nombres.
    /// </para>
    /// </summary>
    private static T? Ascender<T>(Visual? d) where T : Visual
    {
        while (d != null && d is not T) d = d.GetVisualParent();
        return d as T;
    }
    ...
    /// </summary>
    private void OnTablaSorting(object sender, DataGridColumnEventArgs e)
    {
    ...
    private void PintarCola()
    {
        ...
        visorCola.IsVisible = hay;
        lblColaVacia.IsVisible = !hay;
    ...
    private async Task VaciarCola()
    {
        if (_revision.Cuantos == 0) return;
        if (!await Dialogo.Confirmar(Ventana, Textos.Instancia.OrganizarVaciarCola,
    ...
    {
        if (sender is not Control fe || fe.Tag is not CatalogEpisode ep) return;
    ...
    private async void OnPartirSegmentos(object sender, RoutedEventArgs e)
    {
        ...
        if (!await Dialogo.Confirmar(Ventana, Textos.Instancia.OrganizarPartirTitulo,
    ...
    /// </summary>
    private async Task EnviarRepetidoAPapelera(OrganizarRow fila, string ruta, bool esOtro)
    {
        var nombre = System.IO.Path.GetFileName(ruta);
        var quien = esOtro
            ? Textos.Instancia.OrganizarPapeleraOtro
            : Textos.Instancia.OrganizarPapeleraEste;
        if (!await Dialogo.Confirmar(Ventana, Textos.Instancia.OrganizarPapeleraTitulo,
        ...
        {
            await Dialogo.Aviso(Ventana, Textos.Instancia.OrganizarPapeleraFalloTitulo,
    ...
    private void PedirConfirmacion()
    {
        ...
        if (dudas > 0 || desmarcados > 0)
        {
            ...
            filaNoSeToca.IsVisible = true;
        }
        else filaNoSeToca.IsVisible = false;

        overlayConfirmar.IsVisible = true;
    }
    ...
    private void DeshacerUltimoLote()
    {
        ...
        if (vistaRevision.IsVisible && _filas.Count > 0)
        {
    ...
    /// </para>
    /// </summary>
    private async Task ReproducirFila(OrganizarRow? fila)
    {
        ...
        if (Nube.EsMarcador(fila.RutaActual))
        {
            ...
            if (!await Dialogo.Confirmar(Ventana,
    ...
    private async Task AbrirMemoria()
    {
        if (_decisiones.Count == 0) { Aviso(Textos.Instancia.OrganizarSinDecisiones); return; }

        var r = await Dialogo.Confirmar(Ventana, Textos.Instancia.OrganizarMemoriaTitulo,
    ...
    private async Task Aviso(string mensaje)
    {
        Escribir(mensaje);
        await Dialogo.Aviso(Ventana, Textos.Instancia.OrganizarTitulo, mensaje);
    }
    ...
    private void AnalizarPeliculas()
    {
        ...
        _candidatas.Clear();
        _hechoPelis = null;
        bandaPelis.IsVisible = false;
        btnDeshacerPelis.IsVisible = false;
        lblPiePelis.Text = "";
    ...
    {
        if (sender is not Control fe || fe.Tag is not PeliculaFila fila) return;
    ...
    private void PintarRiesgoPeliculas()
    {
        ...
        if (avisos.Count == 0)
        {
            cajaRiesgoPelis.IsVisible = false;
            return;
        }
        ...
        cajaRiesgoPelis.IsVisible = true;
    }
    ...
    private void AplicarPeliculas()
    {
        ...
        bandaPelis.IsVisible = true;
        btnDeshacerPelis.IsVisible = _hechoPelis.Movidos.Count > 0;
    ...
    private void DeshacerPeliculas()
    {
        ...
        btnDeshacerPelis.IsVisible = aMedias;
    ...
    private void PintarIdentificar()
    {
        ...
        lblTmdbApagado.IsVisible = !listo;
    }
