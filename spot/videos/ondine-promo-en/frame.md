...
# ADAPTACIÓN OSCURA — leer antes de tocar nada.
# El preset original es «papel crema con tinta oscura». Ondine es una app de tema oscuro y el spot
# va en el hero de una landing oscura: invertir no es una preferencia, es obligatorio. Se
# conservan los NOMBRES de las claves (el resto del documento las cita) pero se intercambian sus
# papeles: `cream` es ahora el LIENZO (oscuro) e `ink` es el TEXTO (claro).
# Efecto secundario afortunado: `ink@12%` pasa a ser blanco a baja opacidad, que es exactamente
# como se dibuja una hairline en una interfaz oscura. Las sombras sí se repintan a negro: una
# sombra clara no existe.
colors:
  ink: "#E9E9ED"          # texto — antes era el color oscuro
  cream: "#161826"        # LIENZO: el fondo de la app
  tile: "#232532"         # superficie / tarjetas
  tile-strong: "#292B31"  # campos
  coral: "#968AE0"        # el acento, la «voltage» escasa
  navy: "#101120"         # la superficie más honda, para paneles de código
  navy-soft: "#1A1B28"
  navy-elev: "#232532"

borders: { hairline: "1px solid ink@12%", hairline-strong: "1px solid ink@20%", dark: "1px solid ink@14% (on navy)" }
shadows: { card: "0 1px 3px #00000059, 0 8px 28px #00000040", none: "none" }
